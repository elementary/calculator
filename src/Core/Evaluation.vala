...
namespace Calculus.Core {
    ...
    public class Evaluation : Object {
        ...
        private struct Operator { string symbol; int prec; string fixity; Eval eval;}
        private Operator[] operators = {   Operator () { symbol = "+", prec = 1, fixity = "LEFT", eval = (a, b) => { return a + b; } },
                                            ...
                                            Operator () { symbol = "/", prec = 2, fixity = "LEFT", eval = (a, b) => { return a / b; } },
                                            Operator () { symbol = "mod", prec = 2, fixity = "LEFT", eval = (a, b) => { return a % b; } },
                                            Operator () { symbol = "^", prec = 3, fixity = "RIGHT", eval = (a, b) => { return Math.pow (a, b); } },
                                            Operator () { symbol = "e", prec = 4, fixity = "RIGHT", eval = (a, b) => { return a*Math.pow (10, b); } },
                                            Operator () { symbol = "%", prec = 5, fixity = "LEFT", eval = (a, b) => { return b / 100;} } };
        ...
        private List<Token> check_tokens (List<Token> input_tokenlist) throws CHECK_ERROR {
            ...
            tokenlist.append (new Token ("0", TokenType.NUMBER));
        ...
        private Token compute_tokens (Token t_op, Token t1, Token t2) throws EVAL_ERROR {
            try { 
                ...
                var d = op.eval (double.parse (t2.content), double.parse (t1.content));
        ...
        private Token process_tokens (Token tf, Token t1, Token t2) throws EVAL_ERROR {
            try {
                ...
                var d = f.eval (double.parse (t1.content), double.parse (t2.content));
                return new Token (d.to_string (), TokenType.NUMBER);
